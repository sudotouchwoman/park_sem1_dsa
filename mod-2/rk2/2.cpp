...
public:
    ...
    size_t ShortestRootTreeDist() const;
};
...
template<typename T, typename LT>
size_t BinTree<T, LT>::ShortestRootTreeDist() const {
    if (root == nullptr) return 0lu;

    std::stack<std::pair<Node*, size_t>> nodes_to_walk;
    if (root != nullptr) nodes_to_walk.push(std::pair<Node*, size_t>(root, 1lu));

    while (not nodes_to_walk.empty())
    {
        std::pair<Node*, size_t> curr_pair = nodes_to_walk.top();
        auto walker = curr_pair.first;
        auto curr_height = curr_pair.second;
        nodes_to_walk.pop();

        if (walker->left != nullptr) nodes_to_walk.push(std::pair<Node*, size_t>(walker->left, curr_height + 1));
        if (walker->right != nullptr) nodes_to_walk.push(std::pair<Node*, size_t>(walker->right, curr_height + 1));

        if (walker-> left == nullptr and walker->right == nullptr) return curr_height;
    }
    return 0lu;
}
